-- 001_initial.up.sql: Consolidated schema for authserver SQLite backend.
...
CREATE TABLE IF NOT EXISTS revoked_jtis (
    jti        TEXT PRIMARY KEY,
    revoked_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%f', 'now')),
    expires_at TEXT NOT NULL
);
...
CREATE TABLE IF NOT EXISTS resources (
    ...
    -- ON DELETE RESTRICT explicit per audit M1: provider deletion must be blocked
    -- while resources still reference it (the data model). RESTRICT is sqlite's
    -- default but stating it locks the contract against future migrations.
    broker_provider_id  TEXT REFERENCES broker_providers(id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS consent_grants (
    ...
    user_id     TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    client_id   TEXT NOT NULL REFERENCES clients(id) ON DELETE CASCADE,
    -- ON DELETE RESTRICT (audit M1): non-revoked consent grants must block
...
CREATE TABLE IF NOT EXISTS broker_grants (
    id                  TEXT PRIMARY KEY,
    user_id             TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    -- ON DELETE RESTRICT (audit M1): broker_grants must block provider deletion
    -- (the data model — operator deletes a provider only after migrating
    -- or revoking every grant).
    broker_provider_id  TEXT NOT NULL REFERENCES broker_providers(id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS issuances (
    ...
    client_id       TEXT NOT NULL,
    -- ON DELETE RESTRICT (audit M1): non-expired issuances must block resource
...
CREATE TABLE IF NOT EXISTS connect_pending_states (
    ...
    -- ON DELETE RESTRICT (audit M1) for both FKs. connect_pending_states are
    -- ephemeral (10-min TTL) so the FK rarely matters, but if a provider or
    -- resource is deleted while a connect dance is in flight, RESTRICT
    -- forces the operator to wait for expiry rather than orphaning rows.
    provider_id   TEXT NOT NULL REFERENCES broker_providers(id) ON DELETE RESTRICT,
    resource_id   TEXT NOT NULL REFERENCES resources(id) ON DELETE RESTRICT,
...
-- destroying anything.
--
-- scope_map is a JSON-encoded { source_scope: [target_scope, ...] } object
-- stored in a TEXT column (sqlite parity for postgres' JSONB).
