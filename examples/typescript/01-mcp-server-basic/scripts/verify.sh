#!/usr/bin/env bash
# verify.sh — end-to-end smoke for Tier 01 (TypeScript).
#
# Pipeline:
...
# (= the `resource` arg passed to authplaneMcpAuth()). The verifier rejects
# tokens whose `aud` claim does not equal this string byte-for-byte.
...
# DTO: api/admin/dto.go createClientRequest. grant_types is an array; the
...
# Handler: POST /oauth/token. Form parameters: grant_type, scope, resource.
# Client auth via Basic (RFC 6749 §2.3.1).
