#!/usr/bin/env bash
# verify.sh — end-to-end smoke for Tier 01 (Go).
#
# Pipeline:
...
# The Go SDK reads it verbatim from the AUTHPLANE_RESOURCE env var and uses
# it as the JWT `aud` claim the verifier requires; see
...
# Handler: api/public/oauth/handlers.go. Form parameters: grant_type, scope,
# resource. Client auth via Basic (RFC 6749 §2.3.1). Anchor:
