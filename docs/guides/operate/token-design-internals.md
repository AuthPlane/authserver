...
| `admin grant revoke-consent` (`consent_grant.revoked_admin`) | Soft-delete grant; cascade revokes every live Mint issuance minted under it — the client's own and every token exchanged from them, at any depth (`parent_jti` is followed to the leaves); the client's refresh families for the resource are revoked and their access-token JTIs denylisted; introspection reports all of them inactive and a revoked token is refused as a `subject_token` | In-flight tokens until `exp` for resource servers that verify locally; a few more if a cascade half failed (audit row carries `cascade=failed` / `family_cascade=failed`); an authorization code issued before the revoke can still be redeemed within its 10-minute life, and what it mints is not reached until the grant is revoked again |
...
| `admin issuance revoke` (`issuance.revoked_admin`) | One issuance row marked revoked; introspection reports that JTI inactive and the token is refused as a `subject_token` at `/oauth/token` | Until `exp` if the MCP only does local-verify. Tokens already exchanged **from** it are not reached by a single-issuance revoke — revoke the consent grant for that |
