...
- **Audit trail and revocation reach**: every exchange writes an `issuances` row carrying the acting `client_id` (on a fronted exchange, the source resource's slug), the subject user, the resource, the granted scopes, and the lineage: `consent_client_id` — the subject token's client, the one the consent row was keyed on (empty on a fronted exchange, where no consent row is read) — and `parent_jti`, the subject token it was exchanged from. Revoking the user's consent grant follows that lineage: the client's own token for the resource, every token exchanged from it at any depth, and the client's refresh families for the resource are revoked, introspection answers `active: false` for each, and a revoked token is refused as a `subject_token`. The grant is re-read after the mint so a revocation landing mid-exchange refuses the token rather than handing it out. What a resource server sees depends on whether it introspects — see [what revocation reaches](../guides/upstream-providers/token-exchange-grant.md#what-revocation-reaches).
