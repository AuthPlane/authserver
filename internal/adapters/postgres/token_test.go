...
func TestTokenStore(t *testing.T) {
	testdata.RunTokenStoreTests(t, func(t *testing.T) (output.TokenStore, output.ClientStore, output.UserStore) {
		s := testdata.SetupTestPGStores(t, pgContainerDSN)
...
func TestTokenStore_CascadeDelete(t *testing.T) {
	...
	// Delete family directly via pool — refresh tokens should cascade.
	if _, err := db.Pool.Exec(ctx, `DELETE FROM token_families WHERE id = $1`, "fam-cascade"); err != nil {
...
func TestRevocationStore_JTICascadeDelete(t *testing.T) {
	...
	if _, err := db.Pool.Exec(ctx, `DELETE FROM token_families WHERE id = $1`, "fam-jti-cascade"); err != nil {
		t.Fatalf("delete family: %v", err)
	}

	var count int
	if err := db.Pool.QueryRow(ctx,
		`SELECT COUNT(*) FROM access_token_jtis WHERE jti = $1`, "jti-cascade").Scan(&count); err != nil {
...
func TestTokenFamily_CascadeFromClient(t *testing.T) {
	...
	if _, err := db.Pool.Exec(ctx, `DELETE FROM clients WHERE id = $1`, "fk-cli-client"); err != nil {
...
func TestTokenFamily_CascadeFromUser(t *testing.T) {
	...
	if _, err := db.Pool.Exec(ctx, `DELETE FROM users WHERE id = $1`, "fk-usr-user"); err != nil {
...
func TestTokenFamily_FKViolation(t *testing.T) {
	...
	assertPGFKViolation(t, err, "missing client_id")
	...
	assertPGFKViolation(t, err, "missing user_id")
}

// assertPGFKViolation fails the test unless err is a postgres
// foreign_key_violation (SQLSTATE 23503). Bare-`err != nil` is too loose
// because it also passes for unrelated I/O errors.
func assertPGFKViolation(t *testing.T, err error, scenario string) {
	t.Helper()
	if err == nil {
	...
	if !errors.As(err, &pgErr) || pgErr.Code != "23503" {
		t.Errorf("%s: expected pg foreign_key_violation (23503), got: %v", scenario, err)
	}
}
